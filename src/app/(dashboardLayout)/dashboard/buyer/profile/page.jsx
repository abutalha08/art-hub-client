...
export default function BuyerProfilePage() {
  ...
  const onSubmit = async (data) => {
    if (!currentUser?.email) return toast.error("User email not found!");
    ...
    try {
      ...
      // ২. ব্যাকএন্ড এপিআই-তে রিকোয়েস্ট পাঠানো
      ...
      console.error("PROFILE_SUBMIT_ERROR:", error);
  ...
  if (isSessionLoading || (currentUser && isFetchLoading)) {
    return (
      <div className="w-full min-h-screen bg-[#0C0C14] flex flex-col justify-center items-center text-white">
        ...
        <p className="text-sm text-[#8E8E9F] tracking-widest uppercase">Loading Dynamic Profile...</p>
      </div>
    );
  }
  ...
  if (!currentUser) {
    return (
      <div className="w-full min-h-screen bg-[#0C0C14] flex justify-center items-center text-white">
        <p className="text-sm text-red-400">Please log in to manage your profile.</p>
      </div>
    );
  }
      ...
      >
        <div className="mb-8">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[11px] font-semibold uppercase tracking-wider px-2.5 py-1 rounded-md bg-[#27273A] text-[#A78BFA]">
              {dbUser?.role || currentUser?.role || "User"}
            </span>
          </div>
          <p className="text-sm text-[#8E8E9F] mt-1">
            Update your account details and profile image directly from database.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          {(dbUser?.subscription || currentUser?.subscription) && (
            <div className="p-4 rounded-xl bg-[#12121C] border border-[#27273A]/50 flex justify-between items-center text-sm">
              <span className="text-[#8E8E9F]">Current Plan:</span>
              <span className="font-mono text-white font-bold uppercase tracking-wider text-xs bg-[#221c38] px-3 py-1 rounded-lg border border-[#7928CA]/30">
                {(dbUser?.subscription || currentUser?.subscription).replace("_", " ")}
              </span>
            </div>
          )}
